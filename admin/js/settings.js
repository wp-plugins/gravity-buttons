jQuery(document).ready(function($){
	...
	function ViewModel() {
		...
		self.destroyPluginData = function(){
			...
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'gb_admin_ajax',
					endpoint: 'destroy_plugin_data',
					gb_admin_nonce: GB_GLOBALS.GB_ADMIN_NONCE
				},
		...
		self.subscribe = function(){
			...
			$.ajax({
				...
				data: {
					EMAIL: $email_address,
					FNAME: gb_settings.fname,
					WEBSITE: gb_settings.website
				},
				success: function(response){
					
					if(response.result == 'success'){
						
						//save subscribed status & current email address (if changed) to gravity buttons
		...
		self.syncData = function(){
			...
			self.settings.email(gb_settings.email);
			self.settings.subscribed(gb_settings.subscribed);
	...
	ko.applyBindings(viewModel, $('#gb-settings')[0]);
